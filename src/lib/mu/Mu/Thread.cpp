...
#define STACK_CAPACTIY 4096
    ...
    void Thread::go()
    {
        ...
        {
            if (!_exception)
                _exception = context()->exceptionObject(exc);
    ...
    void Thread::run(const Node* node, bool wait)
    {
        if (!isApplicationThread())
        {
            assert(!isRunning());
    ...
    const Value Thread::call(const Function* f, Function::ArgumentVector& args, bool returnArguments)
    {
        ...
        {
            if (returnArguments)
                throw UnimplementedFeatureException();
    ...
    void Thread::terminate(bool now)
    {
        if (isApplicationThread())
        {
            _alive = false;
    ...
    //----------------------------------------------------------------------
    void Thread::jumpPointBegin(unsigned int flags)
    {
        JumpPoint jp;
        jp.flags = flags;
    ...
    jmp_buf& Thread::jumpPoint() { return _jumpPoints.back().env; }

    void Thread::jumpPointRestore()
    {
    ...
    void Thread::jumpPointEnd() { _jumpPoints.pop_back(); }

    //----------------------------------------------------------------------

    void Thread::beginActivation(size_t size)
    {
        //
    ...
    void Thread::newStackFrame(size_t size)
    {
        //
        ...
        //	data. This makes it possible to do stack unwinding efficiently
    ...
    bool Thread::isSymbol(const void* p) const
    {
        if (!p)
            return true;
    ...
    bool Thread::isNode(const void* p, int depth) const
    {
        if (MU_GC_PTR base = GC_base((MU_GC_PTR)p))
        {
    ...
    void Thread::backtrace(BackTrace& trace) const
    {
