...
namespace Mu
{
    ...
    PyModule::PyModule(Context* c, const char* name)
        : Module(c, name)
    {
    }
        ...
        else if (const Class* ctype = dynamic_cast<const Class*>(t))
        {
            ...
            else if (PyDict_Check(pobj))
            {
                ...
                for (size_t i = 0; i < n; i++)
                {
                    ...
                    //
                    //  Since we think PyUnicode_DecodeUTF8 returns a borrowed
                    //  reference, don't decref.
                    //
                    Py_XDECREF(key);
                }
    ...
    NODE_IMPLEMENTATION(PyModule::nPyObject_CallObject2, Pointer)
    {
        ...
        if (!PyTuple_Check(args))
        {
            ...
            //  need to DecRef newly created tuple.  See comment above for
            //  details.
            //
