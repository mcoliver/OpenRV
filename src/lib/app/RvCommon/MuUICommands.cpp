...
namespace Rv
{
    ...
    static void popupMenuInternal(DynamicArray* array, QPoint& location)
    {
        Session* s = Session::currentSession();
        RvDocument* rvDoc = (RvDocument*)s->opaquePointer();
    ...
    {
      public:
        virtual QString type (const QFileInfo& info) { return
    ...
    */
    ...
    NODE_IMPLEMENTATION(remoteConnections, Pointer)
    {
        ...
        {
            if (sessions[i] == s->name())
                myConnections.push_back(connections[i]);
        }
