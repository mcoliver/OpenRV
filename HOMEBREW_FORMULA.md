# OpenRV Homebrew Formula Reference

This document contains the Homebrew formula for OpenRV, configured for **CY 2025** and **FFmpeg 8** with specific non-free codecs and Blackmagic DeckLink SDK support.

## Homebrew Formula (`openrv.rb`)
...
Save the content above into a file named `openrv.rb`.

### 2. Set DeckLink SDK (Optional)

If you wish to build with Blackmagic DeckLink support, ensure you have the SDK zip file downloaded and set the environment variable:
...
Run the following command to build and install OpenRV from source using the local formula file:

```bash
brew install --build-from-source ./openrv.rb
```

### 4. Link to Applications Folder

After installation, you can create a symlink to make OpenRV appear in your `/Applications` folder:

```bash
ln -s $(brew --prefix openrv)/RV.app /Applications/OpenRV.app
```

### 5. Running from CLI

You can launch OpenRV directly from your terminal using:
